var productos=["galletas","pepsi","chicles","chocolate","boing","panditas","yogurt","picafresa"];
var precios=[25,24,1,12,15,16,18,2];
...
const imprimirTabla=()=>{
    ...
    carrito.forEach(item=>{
        ...
        <td><button class="btn btn-danger" onclick="eliminarProducto(${index[0]})">Delete</button></td>
        </tr>
        `
        index++;
    ...
    <td><h3>$ ${total}.00</h3></td>
     <td><button class="btn btn-success" onclick="pagarr(${index})">pagar</button></td>
    </tr>`
   index++;
...
const pagarr=async()=>{
 ...
 const verProductos=()=>{
   ...
   productos.forEach(item=>{
       ...
       <td>${productos[item[0]]}</td>
       <td><button class="btn btn-danger m-2 w-50" value="Borrar" onclick="bProducto(${index})"></button></td>
 ...
 const bProducto=(index)=>{
   Swal.fire({
      title: "ESTAS SEGURO DE QUE QUERER ELIMINAR EL ELEMENTO?",
      showDenyButton: true,
      showCancelButton: false,
      confirmButtonText: "Si",
      denyButtonText: "No"
